...
public class BerlekampWelchDecoder extends PolySolver {
	...
	private void prepareQx(int[] x) {
		...
		for (int i = 0; i < x.length; i++)
			for (int j = 0; j < t; j++)
				matrix[i][j] = GF256Math.pow(x[i], j);
	}
	/**
	...
	private void prepareEx(int[] y) {
		...
		for (int i = 0; i < y.length; i++)
			for (int j = t; j < y.length; j++)
				matrix[i][j] = GF256Math.mult(y[i], GF256Math.pow(x[i], j-t));
	}
	...
	public int[] solve(int[] y) {
		...
		prepareEx(y);
		GF256Matrix decodeMatrix = new GF256Matrix(matrix).computeInverseElimDepRows();
    ...
    private int[] buildMultVector(int x[], int y[], int length) {
    	...
    	for (int i = 0; i < length; i++)
    		res[i] = GF256Math.mult(GF256Math.pow(x[i], f), y[i]);
